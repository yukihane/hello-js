  ...
  switch (req.method) {
    case 'GET':
      ...
      Post.findAll({ order: [['id', 'DESC']] }).then((posts) => {
        ...
        console.info('生成されました: ' + oneTimeToken);
    ...
    case 'POST':
      ...
      req.on('data', (chunk) => {
        ...
        if (!matchResult) {
          console.info('マッチしませんでした');
          util.handleBadRequest(req, res);
        } else {
          const content = matchResult[1];
          const requestedOneTimeToken = matchResult[2];
          console.info('requested: ' + requestedOneTimeToken);
          const stored = oneTimeTokenMap.get(req.user);
          console.info('stored: ' + stored);
          if (stored === requestedOneTimeToken) {
